...
import { fieldTypes, useVisualData } from '@/visual-editor/hooks/useVisualData'
 ...
 * @description 删除实体项
 */
function deleteEntityItem(index: number) {
 ...
 * @description 编辑模型
 */
function editModel(model: VisualEditorModel) {
  console.log(model)
      ...
      <el-collapse-item :title="item.name" :name="item.key">
        <template #title>
          <div class="model-item-title">
            <span class="w-160px truncate">{{ item.name }}</span>
            <div class="model-actions">
              <ElIcon :size="24" color="#2196f3" @click.stop="editModel(item)">
                <Edit />
              </ElIcon>
              <el-popconfirm
                title="确定要删除该模型吗？"
