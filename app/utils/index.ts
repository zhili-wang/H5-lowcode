 ...
 */
// @ts-ignore
export function fileSizeFormatter(row, column, cellValue) {
 ...
 */
export function getUrlValue(key: string, urlStr: string = location.href): string {
  if (!urlStr || !key)
    return ''
 ...
 * @param urlStr 链接地址，默认为当前浏览器的地址
 */
export function getUrlNumberValue(key: string, urlStr: string = location.href): number {
 ...
 * @param order 顺序
 */
export function buildSortingField({ prop, order }) {
 ...
 * @return 求和结果，默认为 0
 */
export function getSumValue(values: number[]): number {
 ...
 * 分转元
 */
export function fenToYuan(price: string | number): string {
 ...
 * @return 格式化后的数量
 */
export function erpNumberFormatter(num: number | string | undefined, digit: number) {
  if (num == null) {
    return ''
  }
 ...
 * @return 格式化后的数量
 */
export function erpCountTableColumnFormatter(_, __, cellValue: any, ___) {
 ...
 * @return 格式化后的数量
 */
export function erpPriceTableColumnFormatter(_, __, cellValue: any, ___) {
 ...
 * @param areaName 地区名称
 */
export function areaReplace(areaName: string) {
  if (!areaName) {
    return areaName
  }
  return areaName
